...
- `CdpTypes` — CDP 协议类型 records（RemoteObject/AXNode/DomNode/QueryMatch）
- `CdpHelpers` — URL 拼接、fetchJson、WS URL 规范化、认证头（含 Extension Relay 集成）
- `CdpClient` — OkHttp WebSocket JSON-RPC 客户端（withCdpSocket 模式）
- `CdpOperations` — 截图/JS 评估/Aria 快照/DOM 快照/CSS 查询/Tab 创建

### Batch 2: Chrome 进程管理

- `RunningChrome` — 运行状态 record
- `ChromeExecutables` — macOS Bundle ID 检测 + Linux which + 自定义路径
- `ChromeProfileDecoration` — Profile JSON 装饰 + clean exit
- `ChromeManager` — 启动（bootstrap→装饰→轮询）/停止（SIGTERM→SIGKILL）/就绪检测

### Batch 3: Extension Relay

- `ExtensionRelayTypes` — 协议消息类型（与 TS Chrome Extension 兼容）
- `ExtensionRelayServer` — Netty HTTP+WS 代理（/extension + /cdp + /json/\*）
- `ExtensionRelayManager` — 端口→实例单例 + relay auth headers

### Batch 4: Playwright 工具集

- `PwToolsCore` — 交互/快照/状态/导航/下载/追踪/对话框
- `PlaywrightSession` — 持久 CDP 连接、Page 状态跟踪、截图

### Batch 5: ServerContext

- `BrowserServerContext` — Profile→Chrome/Playwright/Relay 生命周期管理

### Batch 6: 截图优化

- `ScreenshotNormalizer` — 自适应 resize + JPEG 自适应压缩
- `BrowserConfig` 增强 — getDefaultProfileObj/getProfileList 方法

### Verified

- `mvn clean test -pl openclaw-browser` BUILD SUCCESS — **28 tests**, 0 failures ✅
- 23 source files 编译通过
...
- **MiscMethodRegistrar** — 注册 17 个方法：`talk.mode`、`voicewake.get/set`、`skills.install/update`（简化）+ 12 个 stub（send/poll/tts._/browser.request/web.login._/update.run）
